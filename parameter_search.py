...
# 启用剪枝版本：仅微调两轮参数
# 第一轮 token 限制 N: 20, 30, 40, 50
# 第二轮入围 trace 数 M: 8, 9, 10, 11, 12
PARAMETER_GRID = {
    "first_stage_token_limit": [20, 30, 40, 50],
    "k": list(range(8, 13, 1)),
}

# 固定参数
FIXED_PARAMS = {
    "max_items": 1000 if FIXED_MAX_ITEMS else None,
    "save_interval": 10,
    # 固定其余与投票/关键词相关的参数，专注于剪枝两轮调参
...
def run_single_experiment(config: Dict[str, Any], input_file: str, 
    ...
    metric_abbr = "conf" if config.get('voting_metric', 'confidence') == "confidence" else "sc"
    # 仅展示与剪枝相关的N与M，其他固定
    param_str = f"{metric_abbr}_n{config['first_stage_token_limit']}_m{config['k']}"
    ...
    cmd = [
        ...
        "--k", str(config["k"]),
        # 固定项
        "--voting_metric", str(config.get("voting_metric", "confidence")),
        "--voting_power", str(config.get("voting_power", 0.5)),
        "--keyword_weight_multiplier", str(config.get("keyword_weight_multiplier", 10.0)),
        "--keyword_window_size", str(config.get("keyword_window_size", 50)),
