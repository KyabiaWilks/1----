...
- `dataset_filtering_tools/` - 数据集筛选工具：过滤简单题与清洗数据
- `experimental_pkl_logits/` - 实验性 PKL/logits 提取与分析工具
- `src/` - 源代码模块
- `test_data/` - 测试数据
- `results/` - 实验结果输出目录：保存各次运行的 JSON 汇总与日志

### 运行环境与平台支持

- 建议环境：Python 3.10–3.12，macOS 13+（Intel/Apple Silicon）、Windows 10/11。
- 本项目为纯 Python 项目，无需本地“编译”，按步骤安装依赖后即可运行。

#### Unicode 兼容性（Mac ↔ Windows）
- macOS 终端默认使用 UTF-8，而 Windows 传统控制台常见是 GBK/CP936。两端切换时，JSON/日志/控制台输出可能出现 `UnicodeEncodeError`/`UnicodeDecodeError`。
- 建议：
  - 确保仓库中文件统一为 UTF-8（无 BOM）。
  - 运行时强制 UTF-8：
...
```bash
# 1) 安装/确认 Python（建议 3.11）
brew install python@3.11   # 或使用你已有的 Python3
...
# 5)（首次）获取 openpangu-embedded-7b-model 到项目根目录
git clone https://gitcode.com/ascend-tribe/openpangu-embedded-7b-model.git

# 6) 基本自检
python -c "import json, datasets; print('Env OK')"
...
### 获取 openpangu-embedded-7b-model（默认不在仓库中）

将模型仓库克隆到当前项目“根目录”下（与本 `readme.md` 同级）：

```bash
git clone https://gitcode.com/ascend-tribe/openpangu-embedded-7b-model.git
```

模型仓库地址：[openpangu-embedded-7b-model](https://gitcode.com/ascend-tribe/openpangu-embedded-7b-model.git)
...
```bash
# 运行参数搜索试验（单元/集成测试入口）
python3 test_parameter_search.py
```
