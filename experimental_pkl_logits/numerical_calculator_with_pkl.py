...
class NumericalCalculator:
    ...
    def load_pkl_data(self, pkl_path: str) -> Dict[str, List]:
        """
        从PKL文件加载预计算的推理结果
        ...
        try:
            with open(pkl_path, 'rb') as f:
                pkl_data = CustomUnpickler(f).load()
            print(f"✓ 成功加载PKL文件: {pkl_path}")
            print(f"  包含 {len(pkl_data)} 个问题的推理结果")
            return pkl_data
        except Exception as e:
            print(f"⚠️  加载PKL文件失败: {e}")
            return {}
    ...
    def calculate_confidence_scores(self, outputs: List[str], use_truncated: bool = False, 
                                  ...
                                  cached_token_ids: Optional[List[List[int]]] = None) -> List[float]:
        """
        ...
        Args:
            ...
            cached_token_ids: 预先tokenized的token_ids（可选，用于加速）
    ...
    def calculate_self_certainty(self, outputs: List[str], use_truncated: bool = False, keyword_weight_multiplier: float = 10.0, keyword_window_size: int = 50, cached_token_ids: Optional[List[List[int]]] = None) -> List[float]:
        """
    ...
    def process_data(self, input_file: str, output_file: str = None, save_interval: int = 1, max_items: int = None, k: int = 32, keyword_weight_multiplier: float = 10.0, keyword_window_size: int = 50, pkl_data: Optional[Dict] = None) -> str:
        """
        处理数据文件，实现两阶段流程：
        ...
        Args:
            ...
            k: pruning选择的trace数量
            pkl_data: 从PKL文件加载的预计算数据（可选，用于加速tokenization）
        ...
        for index, item in enumerate(tqdm.tqdm(data, desc="Processing", unit="问题")):
            ...
            # 从PKL提取token_ids（如果可用）
            pkl_token_ids_list = None
            if pkl_data and 'session_id' in item:
                session_id = item['session_id']
                if session_id in pkl_data:
                    pkl_outputs = pkl_data[session_id]
                    if len(pkl_outputs) == len(all_outputs):
                        pkl_token_ids_list = [out.prompt_token_ids for out in pkl_outputs]
            
            # ========== 第一阶段：快速筛选 ==========
            first_stage_confidence = self.calculate_confidence_scores(
                all_outputs, use_truncated=True, 
                keyword_weight_multiplier=keyword_weight_multiplier, 
                keyword_window_size=keyword_window_size,
                cached_token_ids=pkl_token_ids_list
            )
            first_stage_self_certainty = self.calculate_self_certainty(
                all_outputs, use_truncated=True, 
                keyword_weight_multiplier=keyword_weight_multiplier, 
                keyword_window_size=keyword_window_size,
                cached_token_ids=pkl_token_ids_list
            ...
            if pkl_token_ids_list:
                selected_token_ids = [pkl_token_ids_list[i] for i in selected_indices if i < len(pkl_token_ids_list)]
            
            # 对选中的trace计算完整指标
            final_confidence = self.calculate_confidence_scores(
                selected_outputs, use_truncated=False, 
                keyword_weight_multiplier=keyword_weight_multiplier, 
                keyword_window_size=keyword_window_size,
                cached_token_ids=selected_token_ids
            )
            final_self_certainty = self.calculate_self_certainty(
                selected_outputs, use_truncated=False, 
                keyword_weight_multiplier=keyword_weight_multiplier, 
                keyword_window_size=keyword_window_size,
                cached_token_ids=selected_token_ids
            ...
            # 评估正确性（支持数字/字母两种GT格式）
            def _eq(pred, correct):
                if correct is None or pred == "":
                    return False
                gt = _to_letter(correct)
                return gt != "" and pred == gt
            is_correct = _eq(voted_answer, correct_answer)
    ...
    def run_full_pipeline(self,
                          ...
                          keyword_weight_multiplier: float = 10.0,
                          keyword_window_size: int = 50,
                          run_voting: bool = True,
                          pkl_data: Optional[Dict] = None) -> Tuple[str, Optional[str]]:
        """
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="数值计算模块 - 两阶段流程 (支持PKL加速)")
    parser.add_argument("--input_file", type=str, required=True, help="输入JSON文件路径")
    parser.add_argument("--pkl_file", type=str, default=None, help="PKL推理结果文件路径（可选，用于加速tokenization）")
    ...
    pkl_data = None
    if args.pkl_file:
        if os.path.exists(args.pkl_file):
            pkl_data = calculator.load_pkl_data(args.pkl_file)
        else:
            print(f"⚠️  PKL文件不存在: {args.pkl_file}，将使用正常tokenization流程")
